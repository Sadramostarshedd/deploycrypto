...
const App: React.FC = () => {
  ...
  const scheduleBotActivity = useCallback((overrideTeam?: Team) => {
    ...
    for(let i = 0; i < alphaBotCount; i++) {
      votes.push({ 
        team: 'ALPHA', 
        time: Math.floor(Math.random() * 25) + 1, 
  ...
  const fetchUserProfile = async (userId: string) => {
    // FIX: Changed variable 'id' to string literal 'id' for the column filter
  ...
  useEffect(() => {
    const loop = setInterval(async () => {
      ...
      // Sanity check to prevent negative or massive jumps if storage is cleared mid-interval
      if (totalElapsed < 0 || totalElapsed > TOTAL_MATCH_TIME + 10) return;
      ...
      setGameState(prev => {
        ...
        if (currentPhase === 'VOTING' && roundElapsed < 29) {
  ...
  const handleRestart = () => {
    ...
    setGameState({
      ...
      priceSource: gameState.priceSource,
      alphaStats: { ...INITIAL_TEAM_STATS },
      betaStats: { ...INITIAL_TEAM_STATS },
      winner: null,
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4">
          <div className="max-w-md w-full border-2 border-slate-800 bg-slate-900/50 p-8 flex flex-col items-center text-center shadow-2xl shadow-green-500/10">
            ...
            <div className="w-full flex flex-col gap-4">
               <Button onPress={handleRestart} className="h-16 bg-green-500 text-black font-black uppercase tracking-widest rounded-none hover:bg-green-400">START_NEW_OPERATION</Button>
               <Button onPress={handleExit} variant="bordered" className="h-14 border-slate-700 text-slate-400 font-black uppercase tracking-widest rounded-none hover:border-red-500 hover:text-red-500">TERMINATE_CONNECTION</Button>
            </div>
          </div>
        </div>
      )}
