...
import { LogOut, Zap, Crown, Users, Target, Activity, AlertCircle } from 'lucide-react';
...
const GameArena: React.FC<GameArenaProps> = ({ gameState, user, onVote, onSendMessage, onExit, isEligible }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto w-full p-4 flex flex-col h-screen max-h-screen gap-4 overflow-hidden bg-background">
      {/* HEADERBAR */}
      ...
      <div className="grid grid-cols-12 gap-4 flex-1 overflow-hidden min-h-0">
            ...
            <CardBody className="p-6 relative">
              {gameState.phase === 'VOTING' ? (
                ...
                <div className="flex flex-col gap-4">
                   ...
                   <div className="p-3 bg-black/40 border-l-2 border-green-500/50 rounded flex gap-4 items-center">
                      <Zap size={14} className="text-green-500 animate-pulse" />
                      <span className="text-[10px] font-bold text-green-400 uppercase tracking-widest">{gameState.commentary}</span>
                   </div>
                </div>
              )}
            </CardBody>
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-4 overflow-hidden h-full">
           ...
           {/* SIDE-BY-SIDE SQUAD TELEMETRY BOXES (UPPER RIGHT) */}
           ...
           {/* Global Rank sidebar list removed by user request */}
        </div>
      </div>
